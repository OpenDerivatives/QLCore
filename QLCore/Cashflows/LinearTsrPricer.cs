...
namespace QLCore
{
   ...
   */
   public class LinearTsrPricer : CmsCouponPricer, IMeanRevertingPricer
   {
      ...
      {
         if (optionType == Option.Type.Call && effectiveStrike >= shiftedUpperBound_)
            return 0.0;
         if (optionType == Option.Type.Put && effectiveStrike <= shiftedLowerBound_)
            return 0.0;

         // determine lower or upper integration bound (depending on option type)

         double lower = effectiveStrike, upper = effectiveStrike;
         ...
         switch (settings_.strategy_)
         {
            ...
            case Settings.Strategy.VegaRatio:
            {
               ...
               double bound = strikeFromVegaRatio(settings_.vegaRatio_, optionType, effectiveStrike);
               if (optionType == Option.Type.Call)
            ...
            case Settings.Strategy.PriceThreshold:
            {
               ...
               // side of strike
               double bound = strikeFromPrice(settings_.vegaRatio_, optionType, effectiveStrike);
               if (optionType == Option.Type.Call)
                  upper = Math.Min(bound, shiftedUpperBound_);
               else
                  lower = Math.Max(bound, shiftedLowerBound_);
         ...
         result += singularTerms(optionType, effectiveStrike);
