...
namespace QLCore
{
   ...
   public class NumericHaganPricer : HaganPricer
   {
      ...
      public override double optionletPrice(Option.Type optionType, double effectiveStrike)
      {
         ConundrumIntegrand integrand = new ConundrumIntegrand(vanillaOptionPricer_, rateCurve_, gFunction_, fixingDate_, paymentDate_, annuity_, swapRateValue_, effectiveStrike, optionType);
         ...
         if (optionType == Option.Type.Call)
         {
            ...
            integralValue = integrate(effectiveStrike, upperLimit_, integrand);
         }
         else
         {
            a = Math.Min(effectiveStrike, lowerLimit_);
            b = effectiveStrike;
            integralValue = integrate(a, b, integrand);
         }

         double dFdK = integrand.firstDerivativeOfF(effectiveStrike);
         double swaptionPrice = vanillaOptionPricer_.value(effectiveStrike, optionType, annuity_);
